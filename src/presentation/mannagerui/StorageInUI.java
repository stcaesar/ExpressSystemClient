...
public class StorageInUI extends JPanel implements ActionListener{
	...
	private void init(ArrayList<StorageInPO> orderlist) {
	...
	private JTable createTable(ArrayList<StorageInPO> orderlist) {
		JTable table = new JTable();
		String[] tableHeads = new String[]{"单号","入库时间","目的地","区号","排号","架号","位号","状态"};
		...
		for(int i = 0;i<orderlist.size();i++){
			...
			v.add(orderlist.get(i).getintime());
			v.add(orderlist.get(i).getdestination());
			v.add(orderlist.get(i).getdistrict());
			v.add(orderlist.get(i).getrow());
			v.add(orderlist.get(i).getshelf());
			v.add(orderlist.get(i).getnumber());			
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == quitbutton){
			...
			ArrayList<StorageInPO> orderlist=storage.getAllin();
			init(orderlist);
			savebutton.setBounds(390, 5000, 150, 30);
		}
			...
			ArrayList<StorageInPO> orderlist=storage.getAllinunchecked();
			init(orderlist);
			...
			for(int i=0;i<orderlist.size();i++){
				...
				storage.changeinstate(orderlist.get(i).getid());
			}
