...
public class BusinessVouch extends JPanel implements ActionListener{
	...
	public BusinessVouch(){
		setBounds(200,0,900,650);
		setLayout(null);
		titlelabel= new JLabel("收款单查询");
	...
	private void init(ArrayList<BusinessVoucher> business) {
	...
	private JTable createTable(ArrayList<BusinessVoucher> business) {
		...
		for(int i = 0;i<business.size();i++){
			Vector v = new Vector();
			v.add(business.get(i).getReceiptDate());
			v.add(business.get(i).getMoney());
			v.add(business.get(i).getCourier());
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == quitbutton){
			...
			ArrayList<BusinessVoucher> businessBL=business.checkMoney();
			init(businessBL);
			savebutton.setBounds(390, 5000, 150, 30);
		}
		else if(e.getSource() == partbutton){
			BusinessBL business=new BusinessBL();
			ArrayList<BusinessVoucher> businessbl=business.checkVoucher("unchecked");		
			init(businessbl);
